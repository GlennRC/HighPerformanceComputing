...
void magnitude_phase(float* mag, float* phase, float* h_grad, float* v_grad, int width, int height, int offset)
{ 
...
void* process_image(void* arg)
{
    ...
    convolve(v_gradient, v_temps[tid], gbl_img_w, chk_h, g_dv_kernel, 1, gbl_ker_w, offset);

	magnitude_phase(mag_image, phase_image, h_gradient, v_gradient, gbl_img_w, chk_h, offset);
...
int main(int argc, char** argv) {
    ...
    mag_image = (float*)malloc(gbl_img_w*gbl_img_h*sizeof(float));
    phase_image = (float*)malloc(gbl_img_w*gbl_img_h*sizeof(float));
    ...
    for(int i=0; i<p_num; i++) {
        ...
        t_handles[i].tid=i;
        t_handles[i].chk_h=chk_h;
        t_handles[i].offset=offset; 
    ...
    char mag_name[20];
    sprintf(mag_name, "mag.pgm");
    write_image_template(mag_name, mag_image, gbl_img_w, gbl_img_h);

    char phase_name[20];
    sprintf(phase_name, "phase.pgm");
    write_image_template(phase_name, phase_image, gbl_img_w, gbl_img_h);
